...
function runSquash(context, files, hidden) {
  ...
  const appURL = workspace.URLForApplicationWithBundleIdentifier(
    bundleIdentifier
  );
  if (!appURL) {
    showMessage(context, 'Squash not installed');
    workspace.openURL(
      NSURL.URLWithString('https://realmacsoftware.com/squash/?sketch')
    );
    return;
  }
  ...
  if (exportFolder) {
    ...
    potentialExports.forEach(file => {
      ...
      requests.forEach(request => {
        const path = NSString.pathWithComponents([
          exportFolder,
    ...
    exports.forEach(file => {
      if (file.request.format() !== 'svg') {
        const render = MSExporter.exporterForRequest_colorSpace(
          file.request,
